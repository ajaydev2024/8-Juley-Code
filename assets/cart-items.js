...
if (!customElements.get('cart-items')) {
  ...
  connectedCallback() {
     this.querySelectorAll('.cart-item quantity-input input').forEach((each)=> {
        const MinQuantity =   each.getAttribute('min');
        if(parseInt(each.value) < parseInt(MinQuantity)) {
          each.value = MinQuantity
        }
     })
  }
