...
class ProductCard extends HTMLElement {
  ...
  handleSwatchChange(evt) {
    ...
    //  const hoverImage = this.querySelectorAll(`.product-all__images [alt=${evt.target.value}]`) edit by Ajay
      const hoverImage = this.querySelectorAll(`.product-all__images [alt="${evt.target.value}"]`);

      const hoverImageContainer = this.querySelector('.variant-hover__image_container')
      
      if (variantMedia) {
        this.images.forEach((image) => { image.hidden = true;});
        variantMedia.hidden = false;
        hoverImageContainer.innerHTML = hoverImage[1].outerHTML;  // showing 2nd image of on hover of varinats 
      }
    }
...
class ProductRecommendations extends HTMLElement {
  ...
  async init() {
    ...
    try {
      ...
      document.dispatchEvent(new CustomEvent("swym:product-recommendation-loaded"));
    } catch (error) {
